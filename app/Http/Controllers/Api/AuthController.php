...
class AuthController extends Controller {
    ...
    public function register(StoreUserRequest $request, ActivationService $activationService){
    ...
    public function registerFacebook(Request $request, LaravelFacebookSdk $fb) {
        ...
        $data = [
            ...
            'image' => $facebook_user->getPicture()['url']
        ];
        ...
        if(!empty($data['image'])) {
            $user->profile->image()->associate(\App\Models\File::createFromUrl($data['image']));
            $user->push();
        }
    ...
    protected function validator(array $data)
    {
        return Validator::make($data,
            array_merge(
                ...
                ['image_id' => 'exists:'.with(new ProfileImage())->getTable().',id'])
        );
    }
